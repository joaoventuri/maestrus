...
class RelayLink {
  ...
  connect() {
    ...
    const onClose = (a) => {
      // ws (Node) chama (code, reason); browser chama (CloseEvent{code}).
      const code = (a && typeof a === 'object' && typeof a.code === 'number') ? a.code : (typeof a === 'number' ? a : null);
      this._stopHeartbeat(); this._setStatus('offline'); this._failAllPending('connection-closed');
      // 4002 = 'replaced': OUTRA conexão registrou o MESMO did no relay. Se isso
      // repete rápido, são duas máquinas com o mesmo deviceId (config clonada)
      // brigando em loop → avisa o dono pra regenerar o id e parar o flap.
      if (code === 4002) this._noteReplaced();
  ...
  // Conta os 'replaced' (4002) recentes. 3+ em 20s = colisão de did real (duas
  // máquinas com o mesmo deviceId) → dispara onIdentityConflict pra o dono
  // regenerar o id. Um 'replaced' isolado (ex.: host↔discovery da mesma máquina
  // num restart) NÃO dispara — só o padrão de guerra.
  _noteReplaced() {
    const now = Date.now();
    this._replacedAt = (this._replacedAt || []).filter((t) => now - t < 20000);
    this._replacedAt.push(now);
    if (this._replacedAt.length >= 3 && typeof this.opts.onIdentityConflict === 'function') {
      this._replacedAt = [];
      try { this.logger.warn('[relay-link] colisão de deviceId detectada (3x replaced/20s) — regenerando id'); } catch {}
      try { this.opts.onIdentityConflict(); } catch {}
    }
  }
  ...
  // Renova o TOKEN antes de reconectar: após sleep longo o relay_token (TTL
  // ~5-10min) já expirou; reconectar com o token velho dava 4001 → 1 flap
  // offline→online desnecessário. (Igual ao _scheduleReconnect.)
  forceReconnect() {
    if (this.closed) return;
    this._stopHeartbeat();
    ...
    setTimeout(async () => {
      if (this.closed) return;
      if (this.opts.refreshTokenFn) {
        try { const t = await this.opts.refreshTokenFn(); if (t) this.opts.token = t; } catch {}
      }
