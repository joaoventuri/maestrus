...
export interface Project {
  ...
  engine?: 'claude' | 'cloud' | 'codex' | 'codex-api';
  /** Codex: id da thread p/ `codex exec resume` (equivalente ao sessionId do Claude). */
  codexThreadId?: string;
  /** Modo voz ligado: o host injeta a diretriz de fala no system prompt. */
  voiceMode?: boolean;
  isPinned?: boolean;
...
export interface ChatMessage {
  ...
  /** Nota de voz: o que foi enviado ao agente é o texto; o áudio fica pra reouvir. */
  audioUrl?: string;
...
export interface ClaudeEvent {
  ...
  type:
    ...
    | 'done' | 'error' | 'raw' | 'ask-user-question' | 'queue' | 'auth-expired';
  questions?: Array<{ question: string; options: Array<{ label: string; description?: string }>; multiSelect?: boolean }>;
  text?: string;
  name?: string;
  input?: any;
  ...
  subtype?: string;
  exitCode?: number;
  /** No 'done': o turno foi parado pelo usuário, não terminou sozinho. */
  cancelled?: boolean;
  /** No 'queue': a fila de prompts do host, depois de mudar. */
  items?: Array<{ id: string; text: string; at?: number }>;
  /** No 'auth-expired': qual perfil precisa reconectar. */
  profile?: string | null;
  timestamp?: number;
  evt?: any;
}
  ...
  interface Window {
    maestrus: {
      ...
      claudeAuth: {
        ...
        login: (opts?: { console?: boolean; local?: boolean; projectId?: string; profile?: string }) => Promise<{ ok: boolean; code?: number; error?: string }>;
        submitCode: (code: string, opts?: { local?: boolean; projectId?: string }) => Promise<{ ok: boolean; error?: string }>;
        cancel: () => Promise<{ ok: boolean }>;
        onLog: (handler: (payload: { line: string }) => void) => () => void;
      };
      codexAuth: {
        status: () => Promise<{ ok: boolean; loggedIn: boolean; email?: string | null; method?: string | null; error?: string }>;
        login: (opts?: { deviceAuth?: boolean }) => Promise<{ ok: boolean; code?: number; error?: string }>;
        submitCode: (code: string) => Promise<{ ok: boolean; error?: string }>;
        cancel: () => Promise<{ ok: boolean }>;
        logout: () => Promise<{ ok: boolean }>;
        install: () => Promise<{ ok: boolean; installed?: boolean; alreadyPresent?: boolean; error?: string }>;
        onLog: (handler: (payload: { line: string }) => void) => () => void;
      };
      update: {
        check: () => Promise<any>;
