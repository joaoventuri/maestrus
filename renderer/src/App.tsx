...
export default function App() {
    ...
    (async () => {
      ...
      // certo, os projetos do host aparecem; senão cai na tela Remoto pra parear.
        ...
        if (isWeb) {
          ...
          let acc = await window.maestrus.cloud.account().catch(() => null);
  ...
  useEffect(() => {
    ...
    // Graça no "connected": no wake-from-sleep o link faz um forceReconnect que
    // fica ~1s offline antes de voltar online. Sem graça, o banner piscava
    // "conectar → conectado" a cada tampa aberta. Se cair connected=false mas
    // JÁ estava conectado, segura 4s antes de mostrar desconectado — se voltar
    // online nesse meio, cancela. Só reflete queda de verdade (offline > 4s).
    let dropTimer: any = null;
    const wasConnected = { v: false };
    const setConn = (connected: boolean) => {
      if (connected) {
        if (dropTimer) { clearTimeout(dropTimer); dropTimer = null; }
        wasConnected.v = true;
        setClientConnected(true);
      } else if (wasConnected.v) {
        if (!dropTimer) dropTimer = setTimeout(() => { dropTimer = null; wasConnected.v = false; setClientConnected(false); }, 4000);
      } else {
        setClientConnected(false);
      }
    };
    window.maestrus.remote.clientState?.().then((s) => { setConn(!!s.connected); setClientHost(s.hostName); applySync(s); }).catch(() => {});
  ...
  // Disparo: traz a janela e ABRE O MODO VOZ. Só isso.
  //
  // Antes o wake word executava o script de ações do Inicializador e só abria a
  // voz se o script terminasse com um marcador — ou seja, pra falar com o
  // Maestrus você dependia de ter configurado um script certo. Chamar pelo nome
  // e começar a falar é o comportamento que as pessoas já esperam de assistente
  // de voz, e não precisa de configuração nenhuma.
  async function onWakeDetect() {
    try { await window.maestrus.app.showWindow?.(); } catch {}
    const maestrus = projects.find((p) => p.id === 'maestrus') || projects[0];
    if (!maestrus) return;
    setActiveId(maestrus.id);
    setVoiceTarget(maestrus.id);
    setView('chat');
  }
