...
export interface RealtimeStatus {
  ...
  message?: string;
  /** Erro de schema/modelo da Realtime: a sessão nunca vai funcionar → o chat
   *  cai pro modo turn-based em vez de ficar com a voz morta. */
  fatal?: boolean;
}
  ...
  async start(): Promise<{ ok: boolean; error?: string }> {
    ...
    this.offEvent = M()?.realtime?.onEvent?.((ch: string, payload: any) => {
      if (ch === 'realtime:status') {
        ...
        else if (payload?.status === 'error') this.opts.onStatus?.({ status: 'error', message: payload?.message, fatal: !!payload?.fatal });
