  ...
  switch (cmd) {
    ...
    case 'task': {
      ...
      try {
        ...
        // Kanban roda no dispatcher LOCAL desta máquina. Um projeto remoto/cloud
        // (id "remote:<host>:<pid>") gera uma task que NENHUM dispatcher reivindica
        // (o local ignora 'remote', o host usa id simples) → ficava presa em
        // 'ready' pra sempre. Avisa em vez de enfileirar no vazio.
        if (String(tProj.id).startsWith('remote:') || (tProj as any).remoteHostId || (tProj as any).cloud) {
          ctx.pushSystem(`O Kanban roda na máquina que HOSPEDA o projeto. "${tProj.name}" está num host remoto/nuvem — abra o Maestrus nessa máquina (ou no container) pra enfileirar tasks. Aqui você pode enviar prompts normalmente.`);
          return { handled: true };
        }
