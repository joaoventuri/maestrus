...
const LS_MEMBER_WS = 'maestrus_member_ws';      // workspace de MEMBRO escolhido
const REMOTE_TTL = 15 * 24 * 3600 * 1000;

function deviceId(): string {
  let base = localStorage.getItem(LS_DEVICE);
  if (!base) { base = 'web-' + Math.random().toString(36).slice(2) + Date.now().toString(36); localStorage.setItem(LS_DEVICE, base); }
  // Sufixo POR ABA (sessionStorage é por-aba, sobrevive a reload da MESMA aba):
  // duas abas da mesma origem tinham o MESMO did (localStorage compartilhado) →
  // o relay derrubava uma com 4002 'replaced' → cada aba reconectava e derrubava
  // a outra em loop infinito (RPCs falhando, lista piscando). Dids distintos por
  // aba eliminam a colisão na raiz.
  let tab = sessionStorage.getItem('maestrus_tab');
  if (!tab) { tab = Math.random().toString(36).slice(2, 8); sessionStorage.setItem('maestrus_tab', tab); }
  return base + '-' + tab;
}
...
// Handler de log do login do Codex CLI (device-auth por polling no host).
let _codexOnLog: ((p: { line: string }) => void) | null = null;
...
let _migrateCooldownUntil = 0;   // segura migração pra máquina fantasma (ms epoch)
...
// MEMBRO DE WORKSPACE (estilo Notion): quando conectados na instância de um
// DONO do qual somos membro, o relay_token precisa ir com workspace_owner_id
// (o backend assina com o uid do dono). Guardado aqui + em LS pro boot.
let _wsOwnerId: number | null = null;
function loadMemberWs(): any {
...
// Heartbeat do PWA (a cada 20s): o relay/link.js do HOST e o servidor do relay
// já têm ping/pong, mas o WebRelayLink do navegador NÃO tinha — num WAN
// residencial (NAT/timeout de operadora) o socket vira ZUMBI (readyState OPEN
// mas morto) e todo RPC (loadHistory/projects.list) trava 15s e falha, sem
// nada detectar até o usuário reabrir o app ou re-compartilhar no host. Aqui
// pingamos via hostList (respondido pelo próprio relay); se falhar/estourar,
// forçamos reconexão. Só roda com a aba VISÍVEL (no background os wake handlers
// — visibilitychange/focus — já cobrem o retorno, e o timer é estrangulado).
let _hbTimer: any = null;
function ensureHeartbeat() {
  if (_hbTimer || typeof setInterval === 'undefined') return;
  _hbTimer = setInterval(async () => {
    try {
      if (!link || !clientState.connected) return;
      if (typeof document !== 'undefined' && document.visibilityState !== 'visible') return;
      await link.hostList(8000);           // ping leve; relay responde direto
    } catch {
      try { await link?.forceReconnect(); } catch {}
    }
  }, 20000);
}
...
async function refreshProjects() {
  ...
  const r = await link.rpc('projects.list', {}, 8000).catch(() => null);
  // Falha transitória de RPC (host renovando token / blip) → PRESERVA a lista
  // atual em vez de zerar. Sem isso os projetos piscavam/sumiam na reconexão.
  if (!Array.isArray(r)) return cachedProjects;
  cachedProjects = r.map(tag);
  return cachedProjects;
}

// Confirma que o HOST atual responde DE FATO (não só que o WS chegou no relay).
// Um notebook fechado continua listado no relay por um tempo → sem isto o web
// "conectava" nele (host fantasma, sem projetos). projects.list roteia até o
// host; se ele estiver morto, o relay devolve target-offline / dá timeout.
async function verifyHostAlive(timeoutMs: number): Promise<boolean> {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline && !clientState.connected) await new Promise((r) => setTimeout(r, 200));
  if (!link || !clientState.connected) return false;
  try {
    const left = Math.max(2500, deadline - Date.now());
    const r = await link.rpc('projects.list', {}, left);
    ...
    onEvent: (f) => {
      if (f.channel === 'claude' && f.payload) {
        ...
        if (p.type === 'project.removed' && p.projectId) {
          ...
          cachedProjects = cachedProjects.filter((cp) => cp.id !== tid);
...
async function doConnectHost(hostDid: string, hostNameArg?: string): Promise<any> {
  ...
  _wsOwnerId = null; // conexão na PRÓPRIA conta solta o modo membro-de-workspace
...
// ─── Membro de workspace (estilo Notion) ─────────────────────────────────────
// Workspaces onde esta conta é MEMBRO ativo (o dono convidou pelo dashboard).
async function fetchMemberWorkspaces(): Promise<any[]> {
  const a = getAccount(); if (!a) return [];
  try {
    const r = await api('member_workspaces', { license_key: a.licenseKey });
    return (r && r.ok && Array.isArray(r.workspaces)) ? r.workspaces : [];
  } catch { return []; }
}

// Conecta na INSTÂNCIA (container) do dono de um workspace do qual somos
// membro: relay_token com workspace_owner_id → o backend assina com o uid do
// DONO (role client) e entramos na sala do relay dele. O host alvo é o
// container canônico do dono (cloud-u{ownerId}).
async function connectMemberWorkspace(ownerId: number, ownerName?: string): Promise<any> {
  const a = getAccount(); if (!a) return { ok: false, error: 'not_logged_in' };
  const t = await api('relay_token', { license_key: a.licenseKey, device_id: deviceId(), role: 'client', workspace_owner_id: ownerId });
  if (!t.ok || !t.token) return { ok: false, error: t.error || 'no_token' };
  _wsOwnerId = ownerId;
  try { localStorage.setItem(LS_MEMBER_WS, JSON.stringify({ ownerId, ownerName: ownerName || '', ts: Date.now() })); } catch {}
  try { link?.close(); } catch {}
  hostId = 'cloud-u' + ownerId; hostName = ownerName || 'Workspace'; cachedProjects = [];
  startClientLink(t.url, t.token);
  // Espera o WS do relay + a presença REAL do container do dono. Membro é
  // read-only no control plane (não religa container suspenso do dono).
  const d = Date.now() + 10000;
  while (Date.now() < d && !clientState.connected) await new Promise((r) => setTimeout(r, 250));
  const online = await waitHostOnline(hostId, 15000);
  if (online) { await refreshProjects().catch(() => {}); emitProjectsChanged(); }
  else { clientState = { ...clientState, status: 'offline' }; emitClientState(); }
  return { ok: online, hostName, ownerId, memberWorkspace: true, offline: !online };
}
...
async function ensureMaestroHost(): Promise<boolean> {
  ...
  // 2) já atachado a um host cloud (o container) → é o maestro. NÃO sobe sandbox.
  if (link && hostId && isCloudHost(hostId) && clientState.connected) return true;
  clientState = { connected: false, status: 'starting', hostName: 'Maestrus' }; emitClientState();
  // 3) O container por usuário (cloud-u{id}) É o maestro E o host dos projetos.
  //    Conecta NELE — nunca sobe o sandbox legado do cloud_start (que criava um
  //    host paralelo 'cloud-{uid}-maestrus' e deixava projects.create sem rumo).
  try {
    const st: any = await api('container_status', { license_key: a.licenseKey });
    if (st && st.exists && st.container && st.container.device_id) {
      // acorda se suspenso
      if (st.container.status !== 'running') { try { await api('container_provision', { license_key: a.licenseKey }); } catch {} }
      const ok = await ensureHost(st.container.device_id);
      if (ok) return true;
    }
  } catch {}
  // 4) fallback (sem container): sandbox legado por projeto (mantém compat).
...
async function freshClientToken(): Promise<string | null> {
  ...
  try {
    // Em modo membro-de-workspace, o token renovado precisa continuar assinado
    // com o uid do DONO — senão a reconexão cairia na sala errada.
    const body: any = { license_key: a.licenseKey, device_id: deviceId(), role: 'client' };
    if (_wsOwnerId) body.workspace_owner_id = _wsOwnerId;
    const t = await api('relay_token', body);
    return (t && t.ok && t.token) ? t.token : null;
  } catch { return null; }
}
...
async function doDiscover(): Promise<any> {
  ...
  // 1) MÁQUINA: só commita se ela RESPONDER de fato. Um notebook fechado fica
  //    listado no relay por um tempo → antes o web "conectava" nele (host
  //    fantasma, sem projetos) e só um F5 resolvia. Agora, se não responder,
  //    cai pro container.
  if (machine) {
    const did = didOf(machine);
    await doConnectHost(did, machine.name || 'Máquina');
    if (await verifyHostAlive(10000)) return { ok: true, found: 1, hostName: machine.name || 'Máquina', cloud: false };
    // máquina fantasma → desiste dela e tenta o container abaixo
  }
  // 2) CONTAINER cloud (sempre-ligado): o alvo padrão e estável.
  if (container) {
    const did = didOf(container);
    await doConnectHost(did, 'Maestrus Cloud');
    await verifyHostAlive(8000);   // commita mesmo se acordar demorar (24/7)
    return { ok: true, found: 1, hostName: 'Maestrus Cloud', cloud: true };
  }
  // 3) Sem máquina viva nem container: talvez seja MEMBRO de um workspace.
  {
    const wss = await fetchMemberWorkspaces();
    if (wss.length === 1) return connectMemberWorkspace(Number(wss[0].owner_id), wss[0].owner_name || wss[0].owner_email);
    if (wss.length > 1) {
      const saved = loadMemberWs();
      const match = saved && wss.find((w: any) => Number(w.owner_id) === Number(saved.ownerId));
      if (match) return connectMemberWorkspace(Number(match.owner_id), match.owner_name || match.owner_email);
      return { ok: true, found: 0, memberWorkspaces: wss };
    }
...
async function preferMachine(): Promise<any> {
  const a = getAccount(); if (!a) return { ok: false };
  if (_wsOwnerId) return { ok: true, memberWorkspace: true };    // conectado como MEMBRO no workspace do dono — não migra
  ...
  // Cooldown: uma máquina fantasma (notebook fechado ainda listado) não pode ser
  // tentada em loop — se acabou de falhar, fica no container por um tempo.
  if (_migrateCooldownUntil > Date.now()) return { ok: true, cooldown: true };
  const prevHost = hostId, prevName = hostName;
  await doConnectHost(did, machine.name || 'Máquina');
  if (await verifyHostAlive(10000)) return { ok: true, migrated: true, hostName: machine.name };
  // máquina não respondeu → volta pro container e segura migração por 60s
  _migrateCooldownUntil = Date.now() + 60000;
  if (prevHost) { await doConnectHost(prevHost, prevName || 'Maestrus Cloud'); }
  else { await doDiscover(); }
  return { ok: true, ghostMachine: true };
}
...
export function installMaestrusWeb() {
  ...
  (window as any).maestrus = {
    ...
    // Web/PWA: o login do CLI roda no HOST/container, não no navegador. O stub
    // reporta "logado" (os projetos rodam no host); um host sem Codex logado
    // devolve o erro no chat. Login do Codex CLI pelo web é follow-up (device-auth).
    // Login do Codex CLI pelo web/PWA: o HOST/container roda `codex login
    // --device-auth`; o web faz poll de codex.loginState pra pegar a URL/código
    // e o fim. Interface idêntica ao desktop → CodexCliConnect funciona igual.
    codexAuth: {
      status: async () => { if (!link) return { ok: true, loggedIn: false }; try { return await link.rpc('codex.status', {}, 15000); } catch { return { ok: false, loggedIn: false }; } },
      login: async () => {
        if (!link) return { ok: false, error: 'not_connected' };
        await link.rpc('codex.loginStart', { deviceAuth: true }, 15000).catch(() => null);
        let lastLen = 0;
        for (let i = 0; i < 300; i++) { // ~10 min (2s por poll)
          await new Promise((r) => setTimeout(r, 2000));
          let st: any = null;
          try { st = await link.rpc('codex.loginState', {}, 10000); } catch {}
          if (!st) continue;
          if (st.url && _codexOnLog) _codexOnLog({ line: st.url });
          if (st.log && st.log.length > lastLen && _codexOnLog) { _codexOnLog({ line: st.log.slice(lastLen) }); lastLen = st.log.length; }
          if (st.done) return { ok: !!st.ok };
        }
        return { ok: false, error: 'timeout' };
      },
      submitCode: async (code: string) => { if (!link) return { ok: false }; return link.rpc('codex.loginCode', { code }, 15000); },
      cancel: async () => { if (!link) return { ok: false }; return link.rpc('codex.loginCancel', {}, 10000); },
      logout: async () => { if (!link) return { ok: false }; return link.rpc('codex.logout', {}, 15000); },
      install: async () => ({ ok: true, installed: false }), // o host instala no loginStart
      onLog: (handler: (p: { line: string }) => void) => { _codexOnLog = handler; return () => { if (_codexOnLog === handler) _codexOnLog = null; }; },
    },
    ...
    openaiKey: {
      ...
      // Chave em claro pro STT/TTS (o backend é BYOK: exige openai_key no POST).
      get: async () => {
        const cached = (window as any).__maestrusOpenaiKey;
        if (cached) return { ok: true, key: cached as string };
        const a = getAccount(); if (!a) return { ok: false, key: null };
        const r = await api('openai_key_get', { license_key: a.licenseKey }).catch(() => null);
        if (!r?.ok || !r.has_key || !r.enc) return { ok: false, key: null };
        try {
          const pt = await decryptOpenaiKey(r.enc, a.licenseKey);
          if (pt) { (window as any).__maestrusOpenaiKey = pt; return { ok: true, key: pt }; }
        } catch {}
    ...
    cloud: {
      ...
      // SSO por sessão: se o navegador já está logado no site (cookie de sessão
      // PHP, ex.: acabou de cadastrar/logar), pega a conta SEM pedir login de
      // novo. Chamado no boot quando não há conta salva. Same-origin → cookie vai.
      sessionLogin: async () => {
        if (selfhostCfg && (selfhostCfg as any).selfhost) return { ok: false };
        if (getAccount()) return { ok: true, account: getAccount() };
        try {
          const r = await fetch(`${API_BASE}?action=session_login`, {
            method: 'POST', headers: { 'content-type': 'application/json' }, credentials: 'include',
            body: JSON.stringify({ device_id: deviceId(), device_name: (navigator.userAgent || 'web').slice(0, 40) }),
          });
          ...
          const acc = {
            ...
            overageCentsPerGb: data.overage_cents_per_gb || 0, loggedAt: Date.now(),
          };
          setAccount(acc);
      ...
      validate: async () => { const a = getAccount(); if (!a) return { ok: false }; const r = await api('validate', { license_key: a.licenseKey, device_id: deviceId() }); if (r.ok) { const na = { ...a, ...(r.account || {}) }; setAccount(na); return { ok: true, account: na }; } return { ok: false }; },
      ...
      containerPause: async () => { const a = getAccount(); if (!a) return { ok: false }; return api('container_pause', { license_key: a.licenseKey }); },
      containerConnect: async () => autoConnectCloud(),
      // Domínio próprio (plano Pro) da instância cloud. Em self-host o api() é
      // gateado (selfhost_no_cloud) — a UI nem mostra a seção nesse caso.
      domainStatus: async () => { const a = getAccount(); if (!a) return { ok: false }; return api('domain_status', { license_key: a.licenseKey }); },
      domainSet: async (domain: string) => { const a = getAccount(); if (!a) return { ok: false }; return api('domain_set', { license_key: a.licenseKey, domain }); },
      domainVerify: async () => { const a = getAccount(); if (!a) return { ok: false }; return api('domain_verify', { license_key: a.licenseKey }); },
      domainRemove: async () => { const a = getAccount(); if (!a) return { ok: false }; return api('domain_remove', { license_key: a.licenseKey }); },
    },
    // ─── Web Push (PWA) — só pra contas do Maestrus Cloud. No self-host o api()
    // já bloqueia toda chamada externa, então tudo aqui degrada pra { ok:false }.
    push: {
      vapid: async () => api('push_vapid', {}),
      subscribe: async (subscription: any) => {
        const a = getAccount();
        if (!a || a.selfhost || !a.licenseKey || a.licenseKey === 'selfhost') return { ok: false, error: 'not_logged_in' };
        return api('push_subscribe', { license_key: a.licenseKey, subscription });
      },
      unsubscribe: async (endpoint: string) => {
        const a = getAccount();
        if (!a || a.selfhost || !a.licenseKey || a.licenseKey === 'selfhost') return { ok: false };
    ...
    remote: {
      ...
      // Membro de workspace (estilo Notion): lista os workspaces onde esta
      // conta é membro ativo e conecta na instância do dono escolhido.
      memberWorkspaces: async () => fetchMemberWorkspaces(),
      connectWorkspace: async (ownerId: number, ownerName?: string) => connectMemberWorkspace(Number(ownerId), ownerName),
      ...
      onHostState: () => () => {},
      disconnect: async () => { try { link?.close(); } catch {} link = null; hostId = null; _pinnedCloudHost = null; _wsOwnerId = null; cachedProjects = []; try { localStorage.removeItem(LS_REMOTE); localStorage.removeItem(LS_MEMBER_WS); } catch {} clientState = { connected: false, status: 'idle', hostName: null }; emitClientState(); return { ok: true }; },
    ...
    files: {
      uploadToHost: async (projectId: string, att: { name: string; dataB64?: string; path?: string }) => {
        ...
        const b64 = att.dataB64;
        // Arquivo grande: envia EM CHUNKS (o frame do relay é 16MB; um anexo maior
        // derrubava a conexão). Pequeno: um frame só. O host junta e devolve o
        // path com nome seguro.
        if (b64.length <= 3_000_000) {
          return link.rpc('files.upload', { projectId: pid, name: att.name, dataB64: b64 }, 60000);
        }
        const CHUNK = 700_000;
        ...
        for (let i = 0; i < total; i++) {
          last = await link.rpc('files.uploadChunk', { projectId: pid, uploadId, name: att.name, index: i, total, dataB64: b64.slice(i * CHUNK, (i + 1) * CHUNK) }, 60000).catch((e: any) => ({ ok: false, error: String(e && e.message || e) }));
          if (last && last.ok === false) return last;
        }
        return last;
      },
      // Download universal (Fase C): lista e baixa arquivos do workspace do HOST
      // conectado (máquina ou container) — resolve "o arquivo ficou no host".
      tree: async (projectId: string, dir?: string) => {
        if (!link) return { ok: false, error: 'not_connected' };
        const r = parseId(projectId); const pid = r ? r.projectId : projectId;
        return link.rpc('files.tree', { projectId: pid, dir }, 20000).catch((e: any) => ({ ok: false, error: String(e && e.message || e) }));
      },
      pull: async (projectId: string, rel: string) => {
        if (!link) return { ok: false, error: 'not_connected' };
        const r = parseId(projectId); const pid = r ? r.projectId : projectId;
        const first: any = await link.rpc('files.read', { projectId: pid, rel }, 60000).catch((e: any) => ({ ok: false, error: String(e && e.message || e) }));
        if (first && first.ok) return first;
        if (!first || !first.chunked) return first || { ok: false, error: 'no_reply' };
        // Grande: baixa em pedaços e junta.
        const CHUNK = 512 * 1024; let offset = 0; let size = first.size || 0; let mime = 'application/octet-stream';
        const chunks: Uint8Array[] = [];
        for (let guard = 0; guard < 100000; guard++) {
          const c: any = await link.rpc('files.readChunk', { projectId: pid, rel, offset, length: CHUNK }, 60000).catch((e: any) => ({ ok: false, error: String(e && e.message || e) }));
          if (!c || c.ok === false) return c || { ok: false, error: 'chunk_failed' };
          if (c.dataB64) { const bin = atob(c.dataB64); const u = new Uint8Array(bin.length); for (let i = 0; i < bin.length; i++) u[i] = bin.charCodeAt(i); chunks.push(u); }
          size = c.size || size; mime = c.mime || mime; offset = (c.offset || 0) + (c.read || 0);
          if (c.done || (c.read || 0) === 0) break;
        }
        let bin = ''; for (const u of chunks) { let s = ''; for (let i = 0; i < u.length; i++) s += String.fromCharCode(u[i]); bin += s; }
        return { ok: true, rel, size: size || bin.length, mime, dataB64: btoa(bin) };
      },
      // No navegador não há "salvar como" nativo → dispara download via blob.
      saveAs: async (name: string, dataB64: string) => {
        try {
          const bin = atob(String(dataB64 || '')); const u = new Uint8Array(bin.length);
          for (let i = 0; i < bin.length; i++) u[i] = bin.charCodeAt(i);
          const blob = new Blob([u]); const url = URL.createObjectURL(blob);
          const a = document.createElement('a'); a.href = url; a.download = String(name || 'arquivo'); document.body.appendChild(a); a.click();
          setTimeout(() => { URL.revokeObjectURL(url); a.remove(); }, 1000);
          return { ok: true };
        } catch (e: any) { return { ok: false, error: String(e && e.message || e) }; }
      },
    },
    // Powers: agents/comandos/regras globais DO HOST conectado.
    claudePowers: {
    ...
    projects: {
      ...
      delete: async (id: string) => {
        ...
        // Apaga NO host (container) via relay — remove do projectStore + arquivos
        // + sessão. O cloud_delete (backend) não mexia no container, então o
        // projeto voltava no próximo list. Reconecta antes de mandar.
        await ensureHost(r.hostId).catch(() => {});
        try { if (link) await link.rpc('projects.delete', { id: r.projectId }, 30000); }
        catch { /* mesmo se falhar, tira da UI; o broadcast do host reconcilia */ }
      ...
      patch: async (id: string, patch: any) => {
        const r = parseId(id); if (!r) return { ...patch, id };
        // Trocar modelo/thinking/permissão TEM que chegar no host — se a conexão
        // caiu, reconecta antes de mandar; e tenta de novo em target-offline.
        await ensureHost(r.hostId).catch(() => {});
        let updated: any = null;
        for (let attempt = 0; attempt < 2 && link; attempt++) {
          try { updated = await link.rpc('projects.patch', { id: r.projectId, patch }, 8000); break; }
          catch (e: any) {
            if (String(e && e.message || '').includes('target-offline') && attempt === 0) { await ensureHost(r.hostId).catch(() => {}); continue; }
            break;
          }
        }
        // atualiza o cache local (echo do host quando veio; senão otimista)
        const patched = updated ? { model: updated.model, thinkingMode: updated.thinkingMode, permissionMode: updated.permissionMode, engine: updated.engine, name: updated.name } : patch;
        cachedProjects = cachedProjects.map((p) => p.id === id ? { ...p, ...patched } : p);
        // Sinaliza falha explicitamente (patch.__ok=false) pra UI poder avisar,
        // em vez de fingir sucesso silencioso.
        if (updated) return tag(updated);
        return { ...(cachedProjects.find((p) => p.id === id) || cachedStubs.find((p) => p.id === id) || { id }), ...patch, __ok: false };
      },
    },
    ...
    claude: {
      ...
      // Fila de turno — vive no HOST, então aqui é só RPC. Sem estado local:
      // é o que faz a fila ser a mesma no celular e no desktop.
      queueList: async (projectId: string) => {
        const r = parseId(projectId); if (!r || !link) return [];
        await ensureHost(r.hostId);
        return link.rpc('queue.list', { projectId: r.projectId }, 8000).catch(() => []);
      },
      queueAdd: async (projectId: string, text: string, attachments?: any) => {
        const r = parseId(projectId); if (!r || !link) return null;
        await ensureHost(r.hostId);
        return link.rpc('queue.enqueue', { projectId: r.projectId, text, attachments }, 8000).catch(() => null);
      },
      queueRemove: async (projectId: string, itemId: string) => {
        const r = parseId(projectId); if (!r || !link) return false;
        return link.rpc('queue.remove', { projectId: r.projectId, itemId }, 8000).catch(() => false);
      },
      queueReorder: async (projectId: string, ids: string[]) => {
        const r = parseId(projectId); if (!r || !link) return false;
        return link.rpc('queue.reorder', { projectId: r.projectId, ids }, 8000).catch(() => false);
      },
      queueClear: async (projectId: string) => {
        const r = parseId(projectId); if (!r || !link) return false;
      ...
      // Slash-commands roteados pro HOST (antes eram stubs {}/noop → no web
      // /usage, /agents, /compact, /memory "não funcionavam" apesar do host
      // implementar tudo). Espelham as assinaturas do preload desktop.
      // Watchdog: pergunta ao host se o turno ainda roda (destrava o "pensando").
      isBusy: async (projectId: string) => {
        const r = parseId(projectId); if (!r || !link) return { busy: false, known: false };
        await ensureHost(r.hostId);
        const res: any = await link.rpc('claude.status', { projectId: r.projectId }, 6000).catch(() => null);
        return res && typeof res.busy === 'boolean' ? { busy: !!res.busy, known: res.known !== false } : { busy: false, known: false };
      },
      usage: async (opts?: any) => { if (!link) return {}; return link.rpc('claude.usage', opts || {}, 20000).catch(() => ({ ok: false })); },
      version: async () => { if (!link) return 'remote'; return link.rpc('claude.version', {}, 10000).catch(() => 'remote'); },
      logout: async () => { if (!link) return { ok: false }; return link.rpc('claude.logout', {}, 15000).catch(() => ({ ok: false })); },
      listAgents: async (projectId?: string) => {
        if (!link) return [];
        const r = projectId && projectId !== 'maestrus' ? parseId(projectId) : null;
        return link.rpc('claude.listAgents', { projectId: r ? r.projectId : undefined }, 15000).catch(() => []);
      },
      ...
      compact: async (projectId: string, opts?: any) => {
        ...
        await ensureHost(r.hostId);
        return link.rpc('claude.compact', { projectId: r.projectId, focus: opts?.focus }, 180000).catch((e: any) => ({ ok: false, error: String(e && e.message || e) }));
      },
