...
export default function RemoteAccess({ onConnected }: { onConnected?: () => void }) {
  ...
  // Workspaces onde esta conta é MEMBRO (convidada por um dono Pro): a lista
  // vem do backend e conectar entra na instância do dono via relay.
  ...
  useEffect(() => {
    (window.maestrus.remote as any).memberWorkspaces?.()
      .then((w: any) => {
        ...
        setMemberWs(list);
        // Membro sem conexão ativa: leva direto pra aba onde o seletor de
        // workspace aparece (senão ficaria escondido atrás da aba Cloud).
        if (list.length > 0) {
  ...
  // A gestão da INSTÂNCIA cloud (ligar/pausar/domínio) vive na tela "Maestrus
  // Cloud" (CloudScreen). No modelo de container-por-usuário não há mais
  // sandbox por projeto pra ligar/desligar aqui.
      ...
      <div className="remote-stack remote-web">
        ...
        {tab === 'connect' && (
          <>
            ...
            {/* Workspaces onde sou MEMBRO (convidado por um dono Pro) */}
