...
import { modelsForEngine, getModelInfo, costTier } from '../lib/model-info';
import { useT } from '../lib/i18n';

interface Props {
  value: ModelChoice;
  onChange: (m: ModelChoice) => void;
  /** Engine ativa — filtra os modelos por provedor (Claude vs OpenAI/Codex). */
  engine?: string;
}
...
export default function ModelPicker({ value, onChange, engine }: Props) {
  ...
  function toggle() {
    if (!open && triggerRef.current) {
      ...
      // Abre pra CIMA quando o gatilho está perto do fundo (input no rodapé) —
      // senão o menu sai da tela. Ancora pelo bottom nesse caso.
  ...
  return (
    <div className="picker">
      ...
      {open && coords && createPortal(
        <div ref={menuRef} className="picker-menu wide" style={{ top: coords.top, bottom: coords.bottom, right: coords.right }}>
