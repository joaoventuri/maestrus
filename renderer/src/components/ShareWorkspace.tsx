                ...
                <div className="share-pid-list">
                  {localProjects.map((p) => {
                    ...
                    return (
                      <div
                        key={p.id}
                        className={`share-pid-item ${on ? 'on' : ''}`}
                        role="switch"
                        aria-checked={on}
                        tabIndex={0}
                        onClick={() => togglePid(p.id)}
                        onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); togglePid(p.id); } }}
                      >
                        <span>{p.name}</span>
                        <span className={`ui-switch ${on ? 'on' : ''}`}><span className="ui-switch-knob" /></span>
                      </div>
                    );
                  })}
