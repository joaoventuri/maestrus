...
export default function SettingsScreen({ onGoRemote, onModeChange }: { onGoRemote?: () => void; onModeChange?: (mode: 'server' | 'client') => void } = {}) {
  ...
  // CLI (login OAuth no servidor, via Powers → Contas).
  ...
  // Destino da importação: "local" ou o deviceId de um host conectado.
  const [impHosts, setImpHosts] = useState<Array<{ deviceId: string; name: string }>>([]);
  const [impDest, setImpDest] = useState('local');
  const [impToken, setImpToken] = useState('');
  const [uploadPct, setUploadPct] = useState<number | null>(null);
  useEffect(() => {
    if (isWeb) return;
    (window as any).maestrus?.remote?.clientState?.().then((s: any) => {
      if (s?.connected && Array.isArray(s.hosts)) setImpHosts(s.hosts);
    }).catch(() => {});
    const off = (window as any).maestrus?.remote?.onImportProgress?.((d: any) => setUploadPct(typeof d?.pct === 'number' ? d.pct : null));
  ...
  // Codex CLI (engine 'codex' — assinatura ChatGPT)
  const [codexLoggedIn, setCodexLoggedIn] = useState<boolean | null>(null);
  const [codexConnecting, setCodexConnecting] = useState(false);
  // Aba de contas: Claude | Codex (separa os campos de cada provedor).
  const [authTab, setAuthTab] = useState<'claude' | 'codex'>('claude');
  ...
  async function refreshCodex() {
    try { const s: any = await (window as any).maestrus?.codexAuth?.status?.(); setCodexLoggedIn(!!s?.loggedIn); }
    catch { setCodexLoggedIn(false); }
  }
  async function codexLogout() {
    try { await (window as any).maestrus?.codexAuth?.logout?.(); } catch {}
  ...
  useEffect(() => {
    ...
    (window as any).maestrus?.app?.hostState?.().then((r: any) => { setHostRunning(!!r?.running); if (r?.error) setHostErr(String(r.error)); }).catch(() => {});
    (window as any).maestrus?.app?.getLaunchAtLogin?.().then((r: any) => setLaunchLogin(!!r?.enabled)).catch(() => {});
    ...
    refreshCodex(); // desktop = local; web = via host/container (relay)
  }, []);
  ...
  async function toggleHostAlways() {
    ...
    setHostAlways(next); setHostErr(null); setHostBusy(true);
    try {
      const r: any = await window.maestrus.app.setHostAlways?.(next);
      // Reflete o estado REAL: ligou como host de fato? senão mostra o erro.
      if (next) {
        setHostRunning(!!(r && r.hostStarted));
        if (r && r.hostStarted === false) setHostErr(String((r && r.error) || t('mode.hostFailed') || 'Não consegui ligar como host — verifique login/rede.'));
        // Re-checa o estado real algumas vezes (conectando → online) pra o
        // indicador refletir a verdade sem o usuário precisar reabrir a tela.
        for (const ms of [1500, 3500]) {
          setTimeout(async () => {
            try { const s: any = await (window as any).maestrus?.app?.hostState?.(); if (s) { setHostRunning(!!s.running); if (s.running) setHostErr(null); else if (s.error) setHostErr(String(s.error)); } } catch {}
          }, ms);
        }
      } else {
        setHostRunning(false);
      }
    } catch (e: any) { setHostErr(e?.message || 'Erro'); setHostAlways(!next); }
    finally { setHostBusy(false); }
  }

  async function toggleLaunchLogin() {
    const next = !launchLogin;
    setLaunchLogin(next);
    try {
      const r: any = await (window as any).maestrus?.app?.setLaunchAtLogin?.(next);
      if (r && typeof r.enabled === 'boolean') setLaunchLogin(r.enabled);
      else if (r && r.ok === false) setLaunchLogin(!next);
    } catch { setLaunchLogin(!next); }
  }
  ...
  async function importOne(s: ClaudeSession) {
    ...
    try {
      // Destino = host remoto. Com repositório git → clona LÁ (token p/ privado).
      // SEM repositório → cria o projeto vazio no host e só sobe o .jsonl; nada
      // de git nem permissão. Toda sessão pode ir pra nuvem.
      if (impDest !== 'local') {
        const rem = await (window as any).maestrus.cloud.sessionGitRemote?.(s.cwd).catch(() => ({}));
        const repoUrl = rem?.url || null;
        const proj = await (window as any).maestrus.remote.createOnHost(impDest, repoUrl
          ? { name: s.name, source: 'github', repoUrl, gitToken: impToken.trim() || null, localPath: null, mountPath: null }
          : { name: s.name, source: 'none', repoUrl: null, gitToken: null, localPath: null, mountPath: null });
        // Sobe o histórico (.jsonl) local pro projeto no host, em pedaços com
        // progresso em tempo real — senão a conversa chega em branco.
        const mm = /^remote:[^:]+:(.+)$/.exec(proj?.id || '');
        const hostProjectId = mm ? mm[1] : null;
        if (hostProjectId) {
          setUploadPct(0);
          const up = await (window as any).maestrus.remote.uploadSession({ hostId: impDest, projectId: hostProjectId, sessionId: s.sessionId, cwd: s.cwd });
          setUploadPct(null);
          if (up && up.ok === false) { setImporting(null); setImportErr(t('settings.importUploadFail') || ('Falha ao enviar o histórico: ' + (up.error || ''))); return; }
        }
        setImporting(null);
        if (proj) setImported((m) => ({ ...m, [s.sessionId]: true }));
        return;
      }
      ...
      setImportErr(m === 'repo_auth_required'
        ? (t('settings.importRepoAuth') || 'Repositório privado — preencha o token do GitHub acima e importe de novo.')
        : (m || 'erro'));
    }
  }

  // Estilo de resposta: vive no system prompt do host, então vale pra TODOS os
  // projetos e sobrevive a reset de sessão / compact / conversa nova.
  const [style, setStyleState] = useState<string>('concise');
  useEffect(() => {
    (window as any).maestrus?.persona?.get?.().then((r: any) => { if (r?.style) setStyleState(r.style); }).catch(() => {});
  }, []);
  function pickStyle(id: string) {
    setStyleState(id);
    (window as any).maestrus?.persona?.set?.(id).catch(() => {});
  }
  ...
  return (
    <div className="page">
      ...
      <section className="settings-section">
        <h2><MessageSquare size={15} /> {t('settings.styleTitle') || 'Estilo de resposta'}</h2>
        <p className="page-sub">{t('settings.styleDesc') || 'Como o Maestrus responde em todos os projetos. Vale para conversas novas e antigas.'}</p>
        <div className="lang-row">
          <button className={`lang-btn ${style === 'concise' ? 'active' : ''}`} onClick={() => pickStyle('concise')}>
            {t('settings.styleConcise') || 'Direto'}
          </button>
          <button className={`lang-btn ${style === 'detailed' ? 'active' : ''}`} onClick={() => pickStyle('detailed')}>
            {t('settings.styleDetailed') || 'Detalhado'}
          </button>
        </div>
        <p className="page-sub" style={{ marginTop: 8, fontSize: 12 }}>
          {style === 'concise'
            ? (t('settings.styleConciseHint') || 'Respostas curtas, em prosa, sem emoji nem blocos de status. Ignora templates de formatação definidos nos projetos.')
            : (t('settings.styleDetailedHint') || 'Respostas estruturadas, com títulos e listas quando ajudam.')}
        </p>
      </section>
      ...
      {/* Contas & engines em ABAS: Claude (Anthropic) | Codex (OpenAI) */}
      <div className="settings-authtabs">
        <button className={`sat ${authTab === 'claude' ? 'on' : ''}`} onClick={() => setAuthTab('claude')}>
          <ClaudeMark size={14} /> Claude
        </button>
        <button className={`sat sat-codex ${authTab === 'codex' ? 'on' : ''}`} onClick={() => setAuthTab('codex')}>
          <OpenAIMark size={14} /> Codex
        </button>
      </div>
      ...
      {!isSelfhost && (
      <section className="settings-section">
        <h2><ClaudeMark size={15} /> {t('claudeApi.title') || 'Claude API'}</h2>
      ...
      {authTab === 'codex' && (<>
      <section className="settings-section">
        <h2><OpenAIMark size={15} /> {t('codexCli.title') || 'Codex CLI (ChatGPT)'}</h2>
        <p className="page-sub">{t('codexCli.desc') || 'Sign in to Codex with your ChatGPT subscription to use the Codex engine — no API key needed. Maestrus installs the Codex CLI for you.'}</p>
        {codexLoggedIn === null ? (
          <div className="page-sub"><Loader2 size={13} className="spin" /> {t('common.loading') || 'Loading…'}</div>
        ) : codexLoggedIn ? (
          <div className="byok-status">
            <span className="byok-status-ok"><Check size={14} /> {t('codexCli.connectedStatus') || 'Codex CLI connected'}</span>
            <div className="byok-actions">
              <button className="btn-secondary" onClick={codexLogout}>{t('codexCli.logout') || 'Sign out'}</button>
            </div>
          </div>
        ) : (
          <div className="byok-actions">
            <button className="btn-primary" onClick={() => setCodexConnecting(true)}>
              <OpenAIMark size={13} /> {t('codexCli.connect') || 'Connect Codex CLI'}
            </button>
          </div>
        )}
      </section>
      ...
      {codexConnecting && (
        <CodexCliConnect
          onCancel={() => setCodexConnecting(false)}
          onConnected={() => { setCodexConnecting(false); setCodexLoggedIn(true); }}
        />
      )}

      {!isSelfhost && (
      <section className="settings-section">
        <h2><OpenAIMark size={15} /> {t('byok.title') || 'OpenAI — Codex API & Voice (BYOK)'}</h2>
        <p className="page-sub">{t('byok.desc') || 'Use your own OpenAI key to power the Codex API engine and the realtime voice assistant. Encrypted with your license and stored on your Maestrus account — works across desktop, PWA and web.'}</p>
        ...
        >
          ...
          <span className="browser-info">
            <span className="browser-label">
              <Server size={13} /> {t('mode.hostAlwaysTitle')}
              {hostAlways && (
                <span className={`host-dot ${hostBusy ? 'busy' : hostRunning ? 'on' : hostErr ? 'err' : ''}`}>
                  {hostBusy ? (t('mode.hostStarting') || 'ligando…') : hostRunning ? (t('mode.hostOn') || 'ativo') : hostErr ? (t('mode.hostOff') || 'inativo') : ''}
                </span>
              )}
            </span>
            <span className="browser-desc">{t('mode.hostAlwaysDesc')}</span>
            {hostErr && <span className="browser-desc" style={{ color: '#ff7676' }}>{hostErr}</span>}
          </span>
        </button>
        <button
          className={`browser-opt ${launchLogin ? 'active' : ''}`}
          style={{ marginTop: 8, width: '100%' }}
          onClick={toggleLaunchLogin}
        >
          <span className="browser-radio">{launchLogin ? <Check size={13} /> : null}</span>
          <span className="browser-info">
            <span className="browser-label"><Power size={13} /> {t('mode.launchLoginTitle')}</span>
            <span className="browser-desc">{t('mode.launchLoginDesc')}</span>
          </span>
        </button>
      </section>
      ...
      <section className="settings-section">
        ...
        {impHosts.length > 0 && sessions && sessions.length > 0 && (
          <div className="field" style={{ marginTop: 12 }}>
            <span>{t('settings.importDestination') || 'Importar para'}</span>
            <div className="source-tabs">
              <button className={`source-tab ${impDest === 'local' ? 'active' : ''}`} onClick={() => setImpDest('local')}>
                <Laptop size={14} /> {t('settings.importDestLocal') || 'Este computador'}
              </button>
              {impHosts.map((h) => (
                <button key={h.deviceId} className={`source-tab ${impDest === h.deviceId ? 'active' : ''}`} onClick={() => setImpDest(h.deviceId)} title={h.name}>
                  <Server size={14} /> {h.name || 'Host'}
                </button>
              ))}
            </div>
            {impDest !== 'local' && (
              <>
                <small className="page-sub" style={{ marginTop: 6, display: 'block' }}>{t('settings.importHostHint') || 'A conversa fica neste computador; no host o repositório é clonado pra você continuar de qualquer dispositivo.'}</small>
                <input type="password" className="sess-search" style={{ marginTop: 8 }} placeholder={t('settings.importTokenPlaceholder') || 'Token do GitHub (só p/ repo privado) — ghp_… / github_pat_…'} value={impToken} onChange={(e) => setImpToken(e.target.value)} autoComplete="off" />
              </>
            )}
          </div>
        )}
                ...
                {imported[s.sessionId] ? (
                  ...
                  <div className="sess-progress">
                    <span className="sess-progress-label">
                      ...
                      {uploadPct !== null ? `${t('settings.importUploading') || 'Enviando histórico'} ${uploadPct}%` : `${t('settings.import')}…`}
                    </span>
                    {uploadPct !== null && (
                      <span className="sess-progress-bar"><span style={{ width: uploadPct + '%' }} /></span>
                    )}
                  </div>
                ) : (
                  <button className="btn-secondary sess-import" disabled={!s.cwd} onClick={() => importOne(s)}>
                    <DownloadCloud size={13} /> {t('settings.import')}
                  </button>
                )}
              </div>
            ))}
