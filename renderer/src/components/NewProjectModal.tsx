...
import { FolderGit2, HardDrive, Globe, Folder, X, Plug, KeyRound, Lock, CheckCircle2, Loader2, FolderOpen, Laptop, Server } from 'lucide-react';
...
export default function NewProjectModal({ onClose, onCreated }: Props) {
  ...
  // Destino: "local" (este computador) ou o deviceId de um host conectado.
  // Só aparece no desktop com host(s) conectado(s) — no host o projeto é criado
  // LÁ (repo clonado no host; token do git p/ repo privado vai junto).
  const [hosts, setHosts] = useState<Array<{ deviceId: string; name: string; os?: string }>>([]);
  const [dest, setDest] = useState<string>('local');
  useEffect(() => {
    if (isWeb) return;
    (window as any).maestrus?.remote?.clientState?.().then((s: any) => {
      if (s?.connected && Array.isArray(s.hosts)) setHosts(s.hosts);
    }).catch(() => {});
  }, []);
  const onHost = dest !== 'local';
  // Num host, só faz sentido GitHub (clona lá) ou vazio (cria pasta no host) —
  // pasta local/SSH são desta máquina. Se trocar pra host com source inválido, ajusta.
  useEffect(() => {
    if (onHost && (source === 'local' || source === 'production')) setSource('github');
  }, [dest]);
  ...
  async function create() {
    ...
    try {
      const input = {
        ...
        localPath: onHost ? null : (localPath || null),
        mountPath: null,
      };
      // Num host conectado, o projeto é criado LÁ (repo clonado no host, token
      // do git salvo no host). Senão, cria neste computador.
      const project = onHost
        ? await (window as any).maestrus.remote.createOnHost(dest, input)
        : await window.maestrus.projects.create(input);
      onCreated(project);
    } catch (e: any) {
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          {!isWeb && hosts.length > 0 && (
            <div className="field">
              <span>{t('newProject.destination') || 'Onde criar?'}</span>
              <div className="source-tabs">
                <button
                  className={`source-tab ${dest === 'local' ? 'active' : ''}`}
                  onClick={() => setDest('local')}
                ><Laptop size={14} /> {t('newProject.destLocal') || 'Este computador'}</button>
                {hosts.map((h) => (
                  <button
                    key={h.deviceId}
                    className={`source-tab ${dest === h.deviceId ? 'active' : ''}`}
                    onClick={() => setDest(h.deviceId)}
                    title={h.name}
                  ><Server size={14} /> {h.name || 'Host'}</button>
                ))}
              </div>
              {onHost && <small className="hint" style={{ marginTop: 6 }}>{t('newProject.destHostHint') || 'O projeto é criado no host: o repositório é clonado lá e fica acessível de qualquer dispositivo.'}</small>}
            </div>
          )}
