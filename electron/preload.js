...
contextBridge.exposeInMainWorld('maestrus', {
  ...
  app: {
    ...
    hostState: () => ipcRenderer.invoke('app:hostState'),
  ...
  claudeAuth: {
    status: (opts) => ipcRenderer.invoke('claude:authStatus', opts || {}),
    login: (opts) => ipcRenderer.invoke('claude:authLogin', opts || {}),
    submitCode: (code, opts) => ipcRenderer.invoke('claude:authSubmitCode', { code, ...(opts || {}) }),
  ...
  codexAuth: {
    status: () => ipcRenderer.invoke('codex:authStatus'),
    login: (opts) => ipcRenderer.invoke('codex:authLogin', opts || {}),
    submitCode: (code) => ipcRenderer.invoke('codex:authSubmitCode', { code }),
    cancel: () => ipcRenderer.invoke('codex:authCancel'),
    logout: () => ipcRenderer.invoke('codex:authLogout'),
    install: () => ipcRenderer.invoke('codex:authInstall'),
    onLog: (handler) => {
      const fn = (_e, payload) => handler(payload);
      ipcRenderer.on('codex:auth:log', fn);
      return () => ipcRenderer.removeListener('codex:auth:log', fn);
    },
  },
  update: {
  ...
  files: {
    ...
    // Download universal: lista/baixa arquivos do workspace (local OU host remoto).
    tree: (projectId, dir) => ipcRenderer.invoke('files:tree', { projectId, dir }),
    pull: (projectId, rel) => ipcRenderer.invoke('files:pull', { projectId, rel }),
    saveAs: (name, dataB64) => ipcRenderer.invoke('files:saveAs', { name, dataB64 }),
  },
  claudeProfiles: {
    list: () => ipcRenderer.invoke('claudeProfiles:list'),
    // Por projeto: a origem é o dono do projeto (host, container ou local).
    listFor: (projectId) => ipcRenderer.invoke('claudeProfiles:listFor', { projectId }),
    setActiveFor: (projectId, id) => ipcRenderer.invoke('claudeProfiles:setActiveFor', { projectId, id }),
  ...
  openaiKey: {
    ...
    get: () => ipcRenderer.invoke('openaiKey:get'),
  ...
  // Estilo de resposta global do Maestrus (persona persistente).
  persona: {
    get: () => ipcRenderer.invoke('persona:get'),
    set: (style) => ipcRenderer.invoke('persona:set', style),
  },
  claude: {
    ...
    // Fila de turno — mora no HOST, então tudo aqui é ida-e-volta, sem estado local.
    queueList: (projectId) => ipcRenderer.invoke('queue:list', projectId),
    queueAdd: (projectId, text, attachments) => ipcRenderer.invoke('queue:enqueue', { projectId, text, attachments }),
  ...
  browser: {
    ...
    // O agente começou/parou de mexer no navegador → a UI acende o overlay.
    onAgent: (handler) => {
      const fn = (_e, payload) => handler(payload || {});
      ipcRenderer.on('browser:agent', fn);
      return () => ipcRenderer.removeListener('browser:agent', fn);
    },
  },
  cloud: {
    ...
    sessionGitRemote: (cwd) => ipcRenderer.invoke('claude:sessionGitRemote', cwd),
    ...
    containerPause: () => ipcRenderer.invoke('cloud:containerPause'),
    containerConnect: () => ipcRenderer.invoke('cloud:containerConnect'),
    // Domínio próprio (plano Pro) da instância cloud
    domainStatus: () => ipcRenderer.invoke('cloud:domainStatus'),
    domainSet: (domain) => ipcRenderer.invoke('cloud:domainSet', domain),
    domainVerify: () => ipcRenderer.invoke('cloud:domainVerify'),
    domainRemove: () => ipcRenderer.invoke('cloud:domainRemove'),
  },
  tasks: {
  ...
  remote: {
    ...
    createOnHost: (hostId, input) => ipcRenderer.invoke('remote:createOnHost', hostId, input),
    uploadSession: (args) => ipcRenderer.invoke('remote:uploadSession', args),
    onImportProgress: (fn) => {
      const h = (_e, d) => fn(d);
      ipcRenderer.on('remote:importProgress', h);
      return () => ipcRenderer.removeListener('remote:importProgress', h);
    },
