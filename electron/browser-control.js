...
// Operações que só LEEM a página (não mexem em nada) — não acendem o overlay.
const READ_OPS = new Set(['current', 'read', 'html', 'snapshot', 'screenshot']);

// Avisa a UI que o agente está (ou parou de) mexer no navegador. Sem isto, o
// painel só era notificado na PRIMEIRA abertura: navegações seguintes iam
// direto pro webview e a interface não fazia ideia de que algo acontecia.
function notify(active, op) {
  try {
    if (mainWin && !mainWin.isDestroyed()) {
      mainWin.webContents.send('browser:agent', { active, op });
    }
  } catch {}
}
...
async function run(op, args) {
  ...
  const loud = !READ_OPS.has(op);
  if (loud) notify(true, op);
  try {
    return await fn(args || {});
  } finally {
    if (loud) notify(false, op);
  }
}
