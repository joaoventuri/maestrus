...
// Overlay de "estou no controle": um sinal VISÍVEL na tela sempre que o agente
// dirige a máquina. Ponto único — toda operação passa por aqui. Best-effort:
// fora do Electron (ou se falhar) o controle continua normalmente.
let _overlay = null;
try { _overlay = require('./control-overlay'); } catch {}
// screenshot/list_windows/uia_tree são LEITURA — não mexem em nada, então não
// disparam o alarme visual. O overlay é pra ação: clicar, digitar, abrir, focar.
const SILENT_OPS = new Set(['screenshot', 'list_windows', 'uia_tree', 'get_text']);

async function run(op, args = {}) {
  if (_overlay && !SILENT_OPS.has(op)) { try { _overlay.ping('Maestrus is orchestrating'); } catch {} }
  switch (op) {
