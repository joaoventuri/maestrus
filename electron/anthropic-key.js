...
let _plain = null;   // chave decifrada, só em MEMÓRIA (processo)
  ...
  try {
    if (fs.existsSync(vaultPath())) {
      ...
      if (j && typeof j === 'object') {
        // MIGRAÇÃO: versões antigas gravavam `plaintext` no disco (a UI promete
        // "criptografada"). Move pra memória e reescreve o arquivo SEM o plaintext.
...
// Grava SÓ o blob cifrado no disco (enc/updatedAt).
...
// Decifra o enc atual com a licença (sync) e cacheia em memória. É assim que o
// plaintext fica disponível sem persistir no disco.
...
async function fetchAndCache() {
  ...
  const cur = loadCache() || {};
  if (cur.enc === r.enc && _plain) return cur;   // já temos, decifrado em memória
