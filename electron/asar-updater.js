...
async function checkForUpdate() {
  ...
  try {
    ...
    // Só um bump de MAJOR.MINOR do Electron muda a ABI a ponto de o asar antigo
    // ser incompatível → aí sim cai pro instalador completo. Um patch diferente
    // (31.7.6 vs 31.7.7) NÃO quebra o asar (que é JS puro), então comparar a
    // versão EXATA forçava instalação completa à toa toda vez que o CI resolvia
    // um patch diferente (electron era ^31.0.0) — foi o que deu "veio como
    // instalação" na demo. Agora compara só MAJOR.MINOR.
    const mm = (v) => String(v || '').split('.').slice(0, 2).join('.');
    if (meta.electron && mm(meta.electron) !== mm(process.versions.electron)) {
