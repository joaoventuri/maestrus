...
// ─── macOS Keychain → arquivo por perfil ─────────────────────────────────────
// No macOS o Claude CLI guarda o token OAuth no Keychain numa ENTRADA ÚNICA
// (serviço "Claude Code-credentials"), FORA do CLAUDE_CONFIG_DIR. Logo, trocar
// de perfil mudava só o que aparecia (.claude.json), mas o token real era o
// mesmo pra todos os perfis → todo prompt saía de uma conta só. A correção é
// materializar o token no .credentials.json de cada perfil (que o CLI lê, tanto
// no macOS quanto no Linux) e limpar o Keychain, isolando as contas de verdade.
const KEYCHAIN_SERVICE = 'Claude Code-credentials';

// Diretório de config EFETIVO de um perfil (default = ~/.claude).
function effectiveConfigDir(id) {
  const target = id === undefined || id === null ? getActive() : id;
  const dir = configDir(target);
  return dir ? (ensureProfileDir(target) || dir) : path.join(os.homedir(), '.claude');
}

function readKeychainBlob() {
  if (process.platform !== 'darwin') return '';
  try {
    return execFileSync('security', ['find-generic-password', '-s', KEYCHAIN_SERVICE, '-w'], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] }).trim();
  } catch { return ''; } // sem entrada = nada a migrar
}

// Move o token do Keychain pro .credentials.json do perfil DONO (o que está
// usando esse token agora) e apaga a entrada do Keychain. Idempotente: sem
// token no Keychain, é no-op barato. Retorna true se migrou algo.
function migrateKeychainToProfileFile(ownerId) {
  if (process.platform !== 'darwin') return false;
  const blob = readKeychainBlob();
  if (!blob) return false;
  try {
    const dir = effectiveConfigDir(ownerId);
    fs.mkdirSync(dir, { recursive: true });
    fs.writeFileSync(path.join(dir, '.credentials.json'), blob, { mode: 0o600 });
    try { execFileSync('security', ['delete-generic-password', '-s', KEYCHAIN_SERVICE], { stdio: 'ignore' }); } catch {}
    console.log('[claude-profiles] token do Keychain → arquivo do perfil', ownerId || getActive());
    return true;
  } catch (e) { console.warn('[claude-profiles] migração Keychain falhou:', e.message); return false; }
}

// Chamado no boot e antes de cada spawn: garante que o token do perfil ATIVO
// (dono atual do Keychain) vire arquivo. Depois de migrar, o Keychain fica
// vazio e vira no-op. Cada login futuro é migrado no fim do loginStart.
function reconcileKeychainToActive() {
  if (process.platform !== 'darwin') return false;
  return migrateKeychainToProfileFile(getActive());
}
...
function setActive(id) {
  ...
  try { migrateKeychainToProfileFile(s.active); } catch {}
...
function loginStart(id) {
  // Sem id → perfil ATIVO (mesma convenção de status/setActive). O client remoto
  // chama sem id ("logar a conta Claude do host") e antes caía em 'not_found'.
  if (id === undefined || id === null) id = getActive();
  const dir = configDir(id);
  ...
  claudeAuth.login((chunk) => {
    ...
    .then((r) => {
      _login.done = true; _login.ok = !!(r && r.ok); _login.active = false;
      // O `claude auth login` escreveu o token no Keychain (global). Ele é do
      // perfil `id` que acabou de logar → materializa no arquivo dele e limpa o
      // Keychain, pra este perfil ficar isolado dos outros.
      if (_login.ok) { try { migrateKeychainToProfileFile(id); } catch {} }
    })
...
module.exports = {
  ...
  reconcileKeychainToActive, migrateKeychainToProfileFile,
};
