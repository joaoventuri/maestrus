'use strict';
// Powers — gestão dos "superpoderes" dos agentes no host (Claude e Codex):
...
const powersCompile = require('./powers-compile'); // espelha o que dá pro Codex
...
// Depois de mexer nos comandos, recompila a pasta de prompts do Codex inteira
// (é a forma mais simples de refletir criação, edição e remoção de uma vez).
function syncCommandsToCodex() {
  try {
    const items = listDir(commandsDir(), 'command').map((it) => {
      const full = getItem(commandsDir(), it.id);
      return { id: it.id, body: (full && full.body) || '' };
    });
    powersCompile.syncToCodex({ commands: items });
  } catch {}
}
...
const commands = {
  ...
  save: (def) => { const r = saveItem(commandsDir(), def || {}); syncCommandsToCodex(); return r; },
  remove: (id) => { const r = removeItem(commandsDir(), id); syncCommandsToCodex(); return r; },
};
...
const globalMd = {
  ...
  set: (content) => {
    try {
      ...
      // As mesmas regras valem pro Codex, que lê AGENTS.md em vez de CLAUDE.md.
      try { powersCompile.syncToCodex({ rules: content }); } catch {}
      return { ok: true };
    } catch (e) { return { ok: false, error: e.message }; }
  },
};
