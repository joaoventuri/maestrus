...
// codex/codex-api → Codex CLI. A UI e os canais ('claude:*') não mudam.
function ptyFor(project) {
  const e = project && project.engine;
  return (e === 'codex' || e === 'codex-api') ? codexPty : claudePty;
}
const remoteHost = require('./remote-host');
const remoteClient = require('./remote-client');
// did do CLIENT desta máquina no relay: SEMPRE distinto do host (que usa o
// getDeviceId puro). O relay indexa por did e derruba conexões com did igual —
// sem este sufixo, o link de host e o de client da mesma máquina brigavam num
// loop conecta/desconecta. Estável (mesmo -c sempre) → ensureLink idempotente.
function clientDid() { return cloud.getDeviceId() + '-c'; }
...
// Menu enxuto MAS com os roles de edição — sem um menu com esses roles, os
// atalhos Cmd/Ctrl+C·V·X·A NÃO funcionam no macOS (o Electron liga copiar/colar
// aos accelerators do menu). No Windows/Linux a barra fica escondida
// (autoHideMenuBar na criação da janela) mas os accelerators seguem ativos.
(function installAppMenu() {
  const editMenu = { role: 'editMenu' };
  const template = process.platform === 'darwin'
    ? [{ role: 'appMenu' }, editMenu, { role: 'windowMenu' }]
    : [editMenu]; // win/linux: só o Edit (invisível), pros atalhos existirem
...
app.whenReady().then(async () => {
  ...
  setTimeout(() => { try { codexAuth.install(() => {}).catch(() => {}); } catch {} }, 4000);
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        if (!remoteClient.isHealthy(60000)) {
          // SOFT reconnect primeiro: troca o socket SEM zerar hosts/projetos, pra
          // a UI não piscar "nenhum conectado → 2 máquinas" a cada tampa aberta.
          // Só cai pro teardown+subida completa se não houver link (nada a
          // reconectar) — aí precisa reemitir token e re-registrar do zero.
          const soft = remoteClient.reconnect();
          if (!soft || soft.ok === false) {
...
function writeMaestrusMcpConfig(maestrusCodeDir) {
  ...
  // Os mesmos servidores no formato do Codex (~/.codex/config.toml). Best-effort:
  // se o Codex não estiver instalado, não há o que sincronizar.
  try { require('./powers-compile').syncToCodex({ mcpServers: cfg.mcpServers }); } catch {}
...
app.on('before-quit', () => {
  isQuitting = true;
  // O overlay é uma janela sempre-no-topo: se sobrar viva, fica um retângulo
  // laranja fantasma na tela depois de fechar o app.
  try { require('./control-overlay').destroy(); } catch {}
});
...
ipcMain.handle('requirements:install', async (_e, { id }) => {
  ...
  try {
    ...
    // Instalou o Claude → invalida o cache do binário (senão o chat seguia usando
    // o fallback stale até reiniciar o app) e re-aquece a resolução.
    if (id === 'claude' && res && res.ok !== false) {
      try { claudePty.resetClaudeBin(); claudePty.warmClaudeBin(); } catch {}
    }
    return res;
  } catch (e) {
...
// (é lá que o claude roda). Antes checava a máquina LOCAL: o client dizia "sem
// Claude, só Codex" com o host logado, e as telas ficavam dessincronizadas —
// Contas (que já roteava pro host) mostrava uma coisa e o seletor de engine outra.
// `opts.local`: força checar ESTA máquina. O orquestrador "Maestrus" (e o
// Inicializador) rodam SEMPRE local — o host nem os anuncia. Sem esta opção, um
// client perguntava o status do HOST e concluía "Claude ok" pra uma conversa que
// na verdade roda aqui, onde talvez não haja login → o envio falhava sem oferecer
// a tela de conexão. A disponibilidade tem que refletir ONDE O PROJETO RODA.
/**
 * Env do perfil de conta a usar numa operação de auth LOCAL.
 *
 * login e status TEM que resolver isto igual. O CLI deriva o local da
 * credencial (no macOS, o nome da entrada no Keychain) a partir do
 * CLAUDE_CONFIG_DIR: se o login grava no config dir de um perfil e o status
 * lê a credencial base, o login conclui com "Login successful" e a tela
 * insiste que não conectou — foi exatamente o que aconteceu.
 */
function authEnvFor(opts) {
  try {
    const pid = (opts && opts.profile) || claudeProfiles.getActive();
    const ev = claudeProfiles.envVars(pid);
    if (ev && Object.keys(ev).length) return ev;
  } catch {}
  return undefined;
}

ipcMain.handle('claude:authStatus', async (_e, opts) => (opts && opts.local)
  ? claudeAuth.status(authEnvFor(opts))
  // O status tem que perguntar para a MESMA máquina que fez o login, senão o
  // ciclo nunca fecha: o login acontecia local e a confirmação vinha do host.
  : (opts && opts.projectId)
    ? projectProfilesCall(opts.projectId, 'claude.authStatus', {}, () => claudeAuth.status(authEnvFor(opts)))
    : profilesCall('claude.authStatus', {}, () => claudeAuth.status(authEnvFor(opts))));
ipcMain.handle('claude:authLogin', async (_e, opts) => {
  const send = (line) => mainWindow?.webContents.send('claude:auth:log', { line });
  // opts.local: logar NESTA máquina. O orquestrador "Maestrus" roda local, então
  // o login dele tem que abrir o navegador AQUI. Sem isto, o client mandava o
  // login pro host: a tela dizia "abri o navegador" e nada abria no notebook —
  // o navegador abria (ou falhava) do outro lado.
  // Quem roda o turno é quem precisa logar. Antes isto usava uma heurística
  // global: com dois hosts conectados, o login de um projeto LOCAL era enviado
  // para um host — o usuário autenticava no navegador, o token ia parar na
  // máquina errada e a tela dizia "não foi possível conectar".
  const host = (opts && opts.local) ? null
    : (opts && opts.projectId) ? projectHostTarget(opts.projectId)
    : profilesHostTarget();
  if (host) {
    // Client → host: quem precisa logar é o HOST (é lá que o claude roda).
    // Logar localmente não serviria de nada e o status (que vem do host) nunca
    // completaria. Usa o fluxo por polling que já existe em claudeProfiles.
    try {
      await remoteClient.rpc(host, 'claudeProfiles.loginStart', {}, 20000).catch(() => null);
      let lastLen = 0;
      for (let i = 0; i < 300; i++) {
        await new Promise((r) => setTimeout(r, 2000));
        let st = null;
        try { st = await remoteClient.rpc(host, 'claudeProfiles.loginState', {}, 10000); } catch {}
        if (!st) continue;
        if (st.url) send(st.url);
        if (st.log && st.log.length > lastLen) { send(st.log.slice(lastLen)); lastLen = st.log.length; }
        if (st.done) return { ok: !!st.ok };
      }
      return { ok: false, error: 'timeout' };
    } catch (e) { return { ok: false, error: String(e && e.message || e) }; }
  }
  // O login PRECISA escrever no perfil que falhou. Sem isso ele sempre
  // renovava a credencial base (CLAUDE_CONFIG_DIR padrão) enquanto o turno
  // rodava com outro perfil — o usuário autenticava no navegador e o erro
  // continuava, porque a conta renovada não era a que estava em uso.
  const o = { ...(opts || {}) };
  if (!o.env) o.env = authEnvFor(o);
  try { return await claudeAuth.login(send, o); }
  catch (e) { return { ok: false, error: e && e.message || String(e) }; }
});
// Código OAuth colado pelo usuário → stdin do processo de login (do host, se client).
ipcMain.handle('claude:authSubmitCode', async (_e, { code, projectId, local }) => (local
  ? claudeAuth.submitCode(code)
  : projectId
    ? projectProfilesCall(projectId, 'claudeProfiles.loginCode', { code }, () => claudeAuth.submitCode(code))
    : profilesCall('claudeProfiles.loginCode', { code }, () => claudeAuth.submitCode(code))));
ipcMain.handle('claude:authCancel', async () => profilesCall('claudeProfiles.loginCancel', {}, () => ({ ok: claudeAuth.cancelLogin() })));

// ─── Autenticação do Codex CLI (engine 'codex' = assinatura ChatGPT) ────────
// Conectado como CLIENT a um host → gerencia o CODEX DO HOST (mesma regra do
// claudeProfiles): status/login/logout roteiam via relay. profilesCall e
// profilesHostTarget são hoisted (function declarations mais abaixo).
const codexAuth = require('./codex-auth');
ipcMain.handle('codex:authStatus', async () => profilesCall('codex.status', {}, () => codexAuth.status()));
ipcMain.handle('codex:authLogin', async (_e, opts) => {
  const send = (line) => mainWindow?.webContents.send('codex:auth:log', { line });
  const host = profilesHostTarget();
  if (host) {
    // Client → host: dispara device-auth no host e faz POLL do estado (o host
    // não streama onLog pelo relay). Emite URL/log pro overlay como se fosse local.
    try {
      await remoteClient.rpc(host, 'codex.loginStart', { deviceAuth: true }, 20000).catch(() => null);
      let lastLen = 0;
      for (let i = 0; i < 300; i++) {
        await new Promise((r) => setTimeout(r, 2000));
        let st = null;
        try { st = await remoteClient.rpc(host, 'codex.loginState', {}, 10000); } catch {}
        if (!st) continue;
        if (st.url) send(st.url);
        if (st.log && st.log.length > lastLen) { send(st.log.slice(lastLen)); lastLen = st.log.length; }
        if (st.done) return { ok: !!st.ok };
  ...
  try { return await codexAuth.login(send, opts || {}); }
  catch (e) { return { ok: false, error: e && e.message || String(e) }; }
});
ipcMain.handle('codex:authSubmitCode', async (_e, { code }) => profilesCall('codex.loginCode', { code }, () => codexAuth.submitCode(code)));
ipcMain.handle('codex:authCancel', async () => profilesCall('codex.loginCancel', {}, () => ({ ok: codexAuth.cancelLogin() })));
ipcMain.handle('codex:authLogout', async () => profilesCall('codex.logout', {}, () => codexAuth.logout()));
// Auto-instalação do Codex CLI. Conectado a um host, é o host que instala (no
// loginStart) — o client não instala nada localmente.
ipcMain.handle('codex:authInstall', async () => {
  if (profilesHostTarget()) return { ok: true, installed: false };
  const send = (line) => mainWindow?.webContents.send('codex:auth:log', { line });
  try { return await codexAuth.install(send); }
  catch (e) { return { ok: false, error: e && e.message || String(e) }; }
});
...
// Qual host recebe as operações de perfil (list/setActive/login) quando somos
// CLIENT. Bug antigo: só usava getHostId() (primaryHostId), que só é setado em
// pareamento EXPLÍCITO. No modo discovery (container cloud, ou descobrir outra
// máquina) primaryHostId fica null → caía no localFn e a troca de conta era
// aplicada no CLIENT, nunca no HOST → o host seguia na conta antiga. Agora cai
// pro host conectado (único, ou o container cloud). Perfil é global por host.
/**
 * Host que roda ESTE projeto — a conta pertence a quem executa o turno.
 *
 * Antes só existia um alvo global por heurística (host pareado, ou o único, ou
 * o cloud). Com projetos locais e remotos misturados, a lista de contas era a
 * da máquina errada: o cliente via as contas locais achando que eram do host,
 * e trocar não surtia efeito no turno.
 */
function projectHostTarget(projectId) {
  if (!projectId) return profilesHostTarget();
  // O chat de uma conversa usa o id composto `<projectId>#<convId>`; sem tirar
  // o sufixo o projeto não era achado no store e caía no chute lá embaixo.
  const id = String(projectId).split('#')[0];
  try {
    // Projeto remoto SEMPRE carrega o prefixo (remote:/shared:) — é assim que
    // remote-client.js os identifica. Logo, id sem prefixo roda AQUI.
    const h = remoteHostOf(id);
    if (h) return h;
    if (remoteClient.isShared && remoteClient.isShared(id)) return profilesHostTarget();
    if (projectStore.get(id)) return null;
  } catch {}
  // Sem prefixo remoto e sem projeto conhecido: é local. Chutar um host aqui
  // mandava o login de um projeto local para outra máquina — o usuário
  // autenticava no navegador e o token ia parar do lado errado.
  return null;
}

/**
 * Faz a chamada de perfis no dono do projeto (host ou local). Sem fallback
 * silencioso: se o host está offline, é melhor dizer isso do que devolver as
 * contas locais como se fossem dele — foi o que mascarou o bug antes.
 */
async function projectProfilesCall(projectId, channel, payload, localFn) {
  const host = projectHostTarget(projectId);
  if (!host) return localFn();
  try {
    return await remoteClient.rpc(host, channel, payload || {}, 25000);
  } catch (e) {
    return { ok: false, error: 'host_unreachable', detail: String(e && e.message || e) };
  }
}
...
function profilesHostTarget() {
  try {
    ...
    const has = (id) => id && hosts.some((h) => h.deviceId === id);
    const pid = remoteClient.getHostId();
    if (has(pid)) return pid;                                   // pareamento explícito
    if (hosts.length === 1) return hosts[0].deviceId;           // único host (discovery)
    if (hosts.length > 1) {                                     // multi-host: prefere o container cloud
      const cloud = hosts.find((h) => remoteClient.isCloudHost && remoteClient.isCloudHost(h.deviceId));
      return (cloud || hosts[0]).deviceId;
    }
    return pid || null;                                         // fallback: primary mesmo sem estar no mapa
  } catch {}
  return null;
}
async function profilesCall(channel, payload, localFn) {
  ...
  if (host) {
    try { return await remoteClient.rpc(host, channel, payload || {}, 25000); }
      ...
      // Host dormindo/offline (target-offline) ou versão antiga sem este RPC:
      // CAI PRO LOCAL em vez de estourar um erro cru. Sem isto, um host offline
      // impedia até de ver/gerenciar as contas DESTA máquina — que funcionam
      // perfeitamente sem host nenhum.
      console.warn('[maestrus] host indisponível em', channel, '— usando local:', e && e.message);
      try { return await localFn(); } catch { return { ok: false, error: String(e && e.message || e) }; }
    }
  }
  return localFn();
}
// ─── Contas POR PROJETO ─────────────────────────────────────────────────────
// A origem é sempre o dono do projeto, então a mesma tela serve local e remoto
// sem o usuário precisar saber onde cada coisa mora.
ipcMain.handle('claudeProfiles:listFor', async (_e, { projectId }) =>
  projectProfilesCall(projectId, 'claudeProfiles.list', {}, () => claudeProfiles.list()));
ipcMain.handle('claudeProfiles:setActiveFor', async (_e, { projectId, id }) => {
  const r = await projectProfilesCall(projectId, 'claudeProfiles.setActive', { id }, () => claudeProfiles.setActive(id));
  // Avisa TODAS as telas: trocar a conta num lugar tem que refletir em todos
  // os pontos que a mostram (chat, ajustes, banner de limite).
  broadcastProfiles(projectId);
  return r;
});

/** Empurra o estado novo pra janela — a UI não fica pedindo em loop. */
function broadcastProfiles(projectId) {
  try {
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('claude:event', {
        projectId: projectId || '*', type: 'profiles', timestamp: Date.now(),
      });
    }
  } catch {}
}
...
ipcMain.handle('claudeProfiles:setActive', async (_e, { id }) => {
  const r = await profilesCall('claudeProfiles.setActive', { id }, () => claudeProfiles.setActive(id));
  broadcastProfiles(null);
  return r;
});
...
// ─── Powers: agents/comandos/regras globais do HOST ──────────────────
...
// Grava um anexo (blob/base64) em disco pro projeto LOCAL — ex.: imagem colada
// com Ctrl+V não tem path. Vira um arquivo real em <codeDir>/.maestrus/uploads
// (ou tmp) e devolve o path pro @referência funcionar. Mesmo naming do host.
// Materializa um anexo LOCAL num caminho SEGURO dentro do projeto (nome sem
// espaço/acento) — o path original pode ter espaço e quebrar a referência @ do
// Claude ("arquivo não encontrado"). Aceita Buffer ou base64.
function materializeLocalUpload(projectId, name, data) {
  ...
  const safe = raw.replace(/[^A-Za-z0-9._-]+/g, '_').replace(/_+/g, '_').replace(/^_+|_+$/g, '').slice(0, 120) || 'arquivo';
  // Tenta gravar DENTRO do projeto (.maestrus/uploads) pra dar um @path relativo
  // limpo. Se a pasta do projeto for READ-ONLY (ex.: mount OrbStack/aapanel dono
  // root) o mkdir dá EACCES → cai pro tmp em vez de estourar o anexo.
  const tmpBase = path.join(require('os').tmpdir(), 'maestrus-uploads');
  ...
  if (fs.existsSync(dest)) { const ext = path.extname(safe); dest = path.join(baseDir, path.basename(safe, ext) + '-' + Date.now().toString(36) + ext); }
  try { fs.writeFileSync(dest, buf); }
  catch { dest = path.join(tmpBase, safe); fs.mkdirSync(tmpBase, { recursive: true }); fs.writeFileSync(dest, buf); }
  return dest;
}

// Envia um anexo pro host, em CHUNKS quando grande (não estoura o frame de 16MB
// do relay → antes derrubava a conexão / dava "erro" sem explicar). sendRpc é a
// função de RPC já com o alvo (shared/remote) resolvido.
async function uploadToHostChunked(sendRpc, name, b64) {
  const SINGLE_MAX = 3_000_000; // ~2.2MB binário: manda de uma vez (1 round-trip)
  if (b64.length <= SINGLE_MAX) return sendRpc('files.upload', { name, dataB64: b64 });
  const CHUNK = 700_000; // ~512KB binário por frame
  const uploadId = 'u' + Date.now().toString(36) + Math.random().toString(36).slice(2, 8);
  const total = Math.ceil(b64.length / CHUNK) || 1;
  let last = null;
  for (let i = 0; i < total; i++) {
    last = await sendRpc('files.uploadChunk', { uploadId, name, index: i, total, dataB64: b64.slice(i * CHUNK, (i + 1) * CHUNK) });
...
ipcMain.handle('files:uploadToHost', async (_e, { projectId, path: filePath, name, dataB64 }) => {
  try {
    ...
    // LOCAL (este computador): SEMPRE materializa num caminho seguro (copia o
    // arquivo do disco ou grava o blob) → @path sempre limpo, sem espaço.
    if (!isShared && !isRemote) {
      let buf = null;
      if (dataB64) buf = Buffer.from(dataB64, 'base64');
      else if (filePath && fs.existsSync(filePath)) buf = fs.readFileSync(filePath);
      if (!buf || !buf.length) return { ok: false, error: 'no_content' };
      const p = materializeLocalUpload(projectId, name || (filePath ? path.basename(filePath) : 'arquivo'), buf);
      if (!p) return { ok: false, error: 'write_failed' };
      let rel = null;
      try { const proj = projectStore.get(projectId); if (proj && proj.codeDir) { const rr = path.relative(proj.codeDir, p); if (rr && !rr.startsWith('..') && !path.isAbsolute(rr)) rel = rr.split(path.sep).join('/'); } } catch {}
      return { ok: true, local: true, path: p, rel };
    }
    // REMOTO / COMPARTILHADO: lê o conteúdo e sobe (em chunks se grande).
    let b64 = dataB64 || null;
    const fname = name || (filePath ? path.basename(filePath) : 'arquivo');
    if (!b64) {
      if (!filePath || !fs.existsSync(filePath)) return { ok: false, error: 'file_not_found' };
      if (fs.statSync(filePath).size > 50 * 1024 * 1024) return { ok: false, error: 'too_big' };
      b64 = fs.readFileSync(filePath).toString('base64');
    }
    if (isShared) return await uploadToHostChunked((ch, pl) => remoteClient.sharedRpc(projectId, ch, pl, 60000), fname, b64);
    const m = /^remote:([^:]+):(.+)$/.exec(projectId);
    if (!m) return { ok: false, error: 'bad_id' };
    return await uploadToHostChunked((ch, pl) => remoteClient.rpc(m[1], ch, { projectId: m[2], ...pl }, 60000), fname, b64);
  } catch (e) { return { ok: false, error: String(e && e.message || e) }; }
});

// ─── DOWNLOAD universal (Fase C): o client lê os arquivos do workspace onde
// quer que ele viva (local, host remoto ou container). Uma RPC só, resolvida
// aqui — o resto do app (desktop/web/PWA) chama files.tree/read/pull igual.
const fileAccess = require('./file-access');
// Devolve a função de RPC já apontada pro host certo, ou null se o projeto é
// LOCAL (aí lemos direto do disco).
function fileRpcTarget(projectId) {
  const isShared = remoteClient.isShared && remoteClient.isShared(projectId);
  const isRemote = remoteClient.isRemote && remoteClient.isRemote(projectId);
  if (!isShared && !isRemote) return null;
  if (isShared) return (ch, pl) => remoteClient.sharedRpc(projectId, ch, pl, 60000);
  const m = /^remote:([^:]+):(.+)$/.exec(projectId);
  if (!m) return null;
  return (ch, pl) => remoteClient.rpc(m[1], ch, { projectId: m[2], ...pl }, 60000);
}
function localRoot(projectId) { try { const p = projectStore.get(projectId); return (p && p.codeDir) || null; } catch { return null; } }

ipcMain.handle('files:tree', async (_e, { projectId, dir }) => {
  try {
    const send = fileRpcTarget(projectId);
    if (send) return await send('files.tree', { dir });
    return fileAccess.tree(localRoot(projectId), dir);
  } catch (e) { return { ok: false, error: String(e && e.message || e) }; }
});
// Baixa um arquivo COMPLETO (junta os chunks se for grande). Retorna base64.
ipcMain.handle('files:pull', async (_e, { projectId, rel }) => {
  try {
    const send = fileRpcTarget(projectId);
    if (!send) return fileAccess.readFile(localRoot(projectId), rel); // local: direto
    // Remoto: tenta inteiro; se "too_big", baixa em pedaços (offset crescente).
    const first = await send('files.read', { rel });
    if (first && first.ok) return first;
    if (!first || !first.chunked) return first || { ok: false, error: 'no_reply' };
    const CHUNK = 700_000 * 3 / 4 | 0; // ~512KB binário por frame
    let offset = 0, size = first.size || 0, mime = 'application/octet-stream';
    const parts = [];
    for (let guard = 0; guard < 100000; guard++) {
      const r = await send('files.readChunk', { rel, offset, length: CHUNK });
      if (!r || r.ok === false) return r || { ok: false, error: 'chunk_failed' };
      if (r.dataB64) parts.push(r.dataB64);
      size = r.size || size; mime = r.mime || mime;
      offset = (r.offset || 0) + (r.read || 0);
      if (r.done || (r.read || 0) === 0) break;
    }
    // Junta em base64 (via Buffer pra não corromper limites de padding).
    const buf = Buffer.concat(parts.map((b) => Buffer.from(b, 'base64')));
    return { ok: true, rel, size: buf.length || size, mime, dataB64: buf.toString('base64') };
  } catch (e) { return { ok: false, error: String(e && e.message || e) }; }
});
// Salva um arquivo baixado no disco do CLIENT (diálogo "salvar como").
ipcMain.handle('files:saveAs', async (_e, { name, dataB64 }) => {
  try {
    const { dialog } = require('electron');
    const res = await dialog.showSaveDialog({ defaultPath: String(name || 'arquivo') });
    if (res.canceled || !res.filePath) return { ok: false, canceled: true };
    fs.writeFileSync(res.filePath, Buffer.from(String(dataB64 || ''), 'base64'));
...
ipcMain.handle('projects:delete', async (_e, id) => {
  // Projeto num HOST remoto conectado — inclui o container cloud (host
  // cloud-<uid>). Apaga NO host via relay (remove do projectStore + arquivos +
  // sessão) em vez de só sumir localmente. Precede o stub cloud legado, senão
  // "remote:cloud-15:<pid>" cairia no cloudDelete errado e nunca apagaria.
  if (remoteClient.isRemote(id) && remoteClient.isHostConnected(id)) {
    await remoteClient.deleteOnHost(id);
    _cloudStubs = _cloudStubs.filter((s) => s.id !== id); _cloudStubsTs = 0;
...
async function startHost() {
  ...
  // onIdentityConflict: o relay derrubou este host com 4002 'replaced' 3x/20s —
  // OUTRA máquina está usando o MESMO deviceId (config clonada) e as duas brigam
  // em loop (flap infinito → o web não puxa conversas). Regenera um id único e
  // reinicia o host com ele; a máquina "original" mantém o id, o clone diverge.
  let _identityHealing = false;
  const onIdentityConflict = () => {
    if (_identityHealing) return;
    _identityHealing = true;
    try { cloud.regenerateDeviceId(); } catch {}
    try { remoteHost.stop(); } catch {}
...
ipcMain.handle('app:setHostAlways', async (_e, on) => {
  ...
  if (enabled) {
    // Retorna o resultado REAL de subir o host (antes engolia o erro → o toggle
    // ficava "ligado" mas a máquina não virava host de fato). A UI mostra o erro.
    const r = await startHost().catch((e) => ({ ok: false, error: String(e && e.message || e) }));
    const started = !!(r && r.ok !== false) && (() => { try { return !!remoteHost.getState().running; } catch { return true; } })();
    return { ok: started, enabled, hostStarted: started, error: (r && r.error) || null };
  }
  if (_hostRefreshTimer) { clearInterval(_hostRefreshTimer); _hostRefreshTimer = null; }
  try { remoteHost.stop(); } catch {}
  return { ok: true, enabled, hostStarted: false };
});
// Estado REAL do host (a UI reflete "ativo/ligando/erro", não só o setting).
ipcMain.handle('app:hostState', async () => {
  try { const s = remoteHost.getState(); return { alwaysOn: hostAlwaysOn(), running: !!(s && s.running), status: (s && s.status) || 'idle', error: (s && s.error) || null }; }
  catch { return { alwaysOn: hostAlwaysOn(), running: false, status: 'idle' }; }
});

// ── Iniciar o Maestrus COM O COMPUTADOR (login do SO) — ideal p/ servidor/host ─
// Persistido LOCAL (varia por máquina) + aplicado no SO via setLoginItemSettings.
function launchAtLogin() { try { return app.getLoginItemSettings().openAtLogin; } catch { return false; } }
ipcMain.handle('app:getLaunchAtLogin', async () => ({ enabled: launchAtLogin() }));
ipcMain.handle('app:setLaunchAtLogin', async (_e, on) => {
  const enabled = !!on;
  try {
    app.setLoginItemSettings({ openAtLogin: enabled, openAsHidden: false });
    projectStore.setSetting('launch_at_login', enabled);
  } catch (e) { return { ok: false, error: String(e && e.message || e) }; }
  return { ok: true, enabled: launchAtLogin() };
});
...
function maybeAutoHost(attempt = 0) {
  try {
    ...
    startHost().then((r) => {
      ...
      if ((!r || r.ok === false) && attempt < 3) setTimeout(() => maybeAutoHost(attempt + 1), 4000 * (attempt + 1));
    }).catch(() => { if (attempt < 3) setTimeout(() => maybeAutoHost(attempt + 1), 4000 * (attempt + 1)); });
  } catch {}
}
...
// Colisão de deviceId no client (config clonada — outra máquina com o mesmo
// did-c brigando no relay). Regenera o id e reconecta com ele.
let _clientIdentityHealing = false;
remoteClient.setOnIdentityConflict(() => {
  if (_clientIdentityHealing) return;
  _clientIdentityHealing = true;
  try { cloud.regenerateDeviceId(); } catch {}
  try { remoteClient.disconnect(); } catch {}
  setTimeout(() => {
    _clientIdentityHealing = false;
    try {
      let host = null; try { host = projectStore.getSetting('client_host'); } catch {}
      if (host && host.id) startRelayClient(host.id, host.name || '');
      else if (discoveryEnabled && discoveryEnabled()) triggerDiscovery();
    } catch {}
  }, 800);
});
...
async function startRelayClient(hostDeviceId, hostName) {
  const cdid = clientDid();
  const t = await cloud.relayToken('client', cdid);
  if (!t || !t.ok || !t.token) return { ok: false, error: (t && t.error) || 'no_token' };
  const refreshTokenFn = async () => {
...
ipcMain.handle('cloud:containerPause', async () => cloud.containerPause());
// Domínio próprio (plano Pro) da instância cloud
ipcMain.handle('cloud:domainStatus', async () => cloud.domainStatus());
ipcMain.handle('cloud:domainSet', async (_e, domain) => cloud.domainSet(domain));
ipcMain.handle('cloud:domainVerify', async () => cloud.domainVerify());
ipcMain.handle('cloud:domainRemove', async () => cloud.domainRemove());
...
async function startRelayDiscovery() {
  const cdid = clientDid();
  remoteClient.setSelfHostId(cloud.getDeviceId());  // não descobrir a si mesmo
  const t = await cloud.relayToken('client', cdid);
  if (!t || !t.ok || !t.token) return { ok: false, error: (t && t.error) || 'no_token' };
  const refreshTokenFn = async () => {
    const nt = await cloud.relayToken('client', cdid);
...
async function startRelayDiscoveryAsHost() {
  if (!cloud.getAccount || !cloud.getAccount()) return { ok: false, error: 'not_logged_in' };
  // O client desta máquina usa o did -c (distinto do host, que usa o id puro),
  // então NÃO colide com a conexão de host no relay. Antes o '-disc' ia só na
  // query e o relay o ignorava (usava o did do token = id puro) → colisão/loop.
  const did = clientDid();
  remoteClient.setSelfHostId(cloud.getDeviceId());  // não descobrir a si mesmo
  const t = await cloud.relayToken('client', did);
  if (!t || !t.ok || !t.token) return { ok: false, error: (t && t.error) || 'no_token' };
  const refreshTokenFn = async () => {
...
async function triggerDiscovery() {
  if (!cloud.getAccount || !cloud.getAccount()) return { ok: false, error: 'not_logged_in' };
  // Descoberta NÃO é gateada: ela acha o PRÓPRIO container do usuário (incluso
  // no trial) e roda a cada abertura/resume. O gate antigo `count <= 0` era um
  // bug INVERTIDO — bloqueava usuário NOVO (count=0) logo no 1º acesso com
  // "assine o Pro" sem ter feito nada (caso do Cirillo). O paywall de
  // multi-dispositivo fica só no pareamento peer (remote:connect) e no host.
...
ipcMain.handle('remote:reconnect', async () => {
  // Reconectar num host JÁ SALVO (o próprio container/máquina do usuário) não
  // consome amostra nem exige Pro — senão o usuário novo cai no paywall logo no
  // boot em client mode. O mesmo gate `count <= 0` estava INVERTIDO (bloqueava
  // count=0). O paywall multi-dispositivo fica só no pareamento novo por código
  // (remote:connect / pair) e no host.
  let host = null; try { host = projectStore.getSetting('client_host'); } catch {}
...
ipcMain.handle('remote:createOnHost', async (_e, hostId, input) => remoteClient.createOnHost(hostId, input));
ipcMain.handle('remote:uploadSession', async (_e, args) => {
  const { hostId, projectId, sessionId, cwd } = args || {};
  const localPath = sessionScanner.sessionFilePath(cwd, sessionId);
  if (!localPath) return { ok: false, error: 'session_file_not_found' };
  const onProgress = (pct) => { try { mainWindow?.webContents.send('remote:importProgress', { projectId, pct }); } catch {} };
  try { return await remoteClient.uploadSessionToHost(hostId, projectId, sessionId, localPath, onProgress); }
  catch (e) { return { ok: false, error: String(e && e.message || e) }; }
});
...
ipcMain.handle('claude:sessionGitRemote', async (_e, cwd) => ({ url: sessionScanner.gitRemote(cwd) }));
...
// Chave em claro pro renderer usar no STT/TTS (o backend é BYOK: exige a chave
// no POST). Sem isso o voice_stt/realtime_tts voltavam 'no_api_key' e a voz
// falhava em silêncio (STT nunca ouvia; TTS caía no sintetizador nativo).
ipcMain.handle('openaiKey:get', async () => {
  try { const k = openaiKey.getCachedKey() || await openaiKey.getKey(); return { ok: !!k, key: k || null }; }
...
// ─── Fila de turno: o HOST drena, não o navegador ───────────────────────────
// A fila vive em turn-queue.js (disco) e é a mesma em todos os dispositivos.
// Quando um turno termina, o próximo item sai daqui — mesmo que ninguém esteja
// com a conversa aberta, e mesmo que o app tenha sido reiniciado no meio.
const turnQueue = require('./turn-queue');

// Toda mudança na fila vira evento pros clients (janela + relay), no mesmo
// canal dos eventos de turno: quem está olhando vê a fila mudar ao vivo.
turnQueue.setOnChange((projectId, items) => {
  try { claudePty.emit({ projectId, type: 'queue', items, timestamp: Date.now() }); } catch {}
});

claudePty.onEvent((payload) => {
  if (!payload || payload.type !== 'done' || payload.cancelled) return;
  const pid = payload.projectId;
  if (!pid || !turnQueue.count(pid)) return;
  // Pequeno atraso: deixa o 'done' chegar na UI antes do próximo 'user'.
  setTimeout(async () => {
    try {
      if (claudePty.isBusy(pid) || codexPty.isBusy(pid)) return; // alguém correu na frente
...
ipcMain.handle('queue:list', async (_e, projectId) => {
  ...
  return turnQueue.list(projectId);
});
...
ipcMain.handle('queue:remove', async (_e, { projectId, itemId }) => {
  ...
  return turnQueue.remove(projectId, itemId);
});
ipcMain.handle('queue:reorder', async (_e, { projectId, ids }) => {
  if (remoteClient.isRemote(projectId)) return remoteClient.queueCall(projectId, 'queue.reorder', { ids });
  return turnQueue.reorder(projectId, ids);
});
ipcMain.handle('queue:clear', async (_e, projectId) => {
  if (remoteClient.isRemote(projectId)) return remoteClient.queueCall(projectId, 'queue.clear');
  return turnQueue.clear(projectId);
});
...
ipcMain.handle('claude:stop', async (_e, projectId) => {
  ...
  // Não sabemos a engine só pelo id — mata nos dois (só um tem o processo).
  const a = claudePty.kill(projectId); const b = codexPty.kill(projectId);
  return a || b;
});
...
ipcMain.handle('claude:isBusy', async (_e, projectId) => {
  try {
    ...
    if (remoteClient.isRemote(projectId)) { const h = remoteHostOf(projectId); if (h) await ensureRemoteHost(h); return remoteClient.statusOf(projectId); }
    return { busy: !!(claudePty.isBusy(projectId) || codexPty.isBusy(projectId)), known: true };
